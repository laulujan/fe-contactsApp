...
const SearchBar = (props) => {
  ...
  const btnInvisible =
    "invisible px-6 py-3 mt-3 text- text-white transition-all duration-150 ease-linear rounded-full shadow outline-none bg-red-400 hover:bg-red-500 hover:shadow-lg focus:outline-none ";
  const btnVisible =
  ...
  return (
    <div className="flex">
      <div>
        {invisible ? (
          <button className={btnInvisible}>Back</button>
        ) : (
          <button className={btnVisible} onClick={() => onClickBack()}>
            Back
          </button>
        )}
      </div>
